...
class CLeaderboardFindHandler
{
    ...
    {
        m_SteamCallResultFindLeaderboard.Set(apicall, this, &CLeaderboardFindHandler::OnFindLeaderboard);
    }
...
public:
    ...
    CLeaderboardUploadHandler(int _postId)
        : m_postId(_postId)
    {
    }
    ...
    void    SetCallResult(SteamAPICall_t apicall)
    {
        m_SteamCallResultUploadScore.Set(apicall, this, &CLeaderboardUploadHandler::OnUploadResult);
    }
...
public:
    ...
    void    SetCallResult(SteamAPICall_t apicall)
    {
        m_SteamCallResultDownloadScore.Set(apicall, this, &CLeaderboardDownloadHandler::OnDownloadResult);
    }
...
void DoLBRequest(SLBRequest* pRequest, SteamLeaderboard_t hLeaderboard)
{
    if (pRequest->m_bPost)
    {
        ...
        SteamAPICall_t hSteamAPICall = SteamUserStats()->UploadLeaderboardScore(hLeaderboard, k_ELeaderboardUploadScoreMethodKeepBest, pRequest->m_score, pRequest->m_pExtraData, pRequest->m_extraDataCount);
    ...
    {
        //dbg_csol.Output("Sending queued LB get: %s, %d, %d, %d, %d \n", pszName, pRequest->m_pContext->ID, pRequest->m_method, pRequest->m_rangeStart, pRequest->m_rangeEnd);
        SteamAPICall_t hSteamAPICall = SteamUserStats()->DownloadLeaderboardEntries(hLeaderboard, (ELeaderboardDataRequest)pRequest->m_method, pRequest->m_rangeStart, pRequest->m_rangeEnd);
        ...
        CLeaderboardDownloadHandler* pResultHandler = new CLeaderboardDownloadHandler(pRequest->m_postId);// getAsyncRequestInd());// pRequest->m_pContext->ID);//Here...
        pResultHandler->SetCallResult(hSteamAPICall);
    }
}
...
void AddLeaderboardInfo(const char* pszName, bool bRequestCreate)
{
    ...
    if (m_numLeaderboardInfo >= m_MaxLeaderboards)
    {
        ...
        m_aLeaderboardInfo = (SLeaderboardInfo*)g_pYYRunnerInterface->YYRealloc((void*)m_aLeaderboardInfo, sizeof(SLeaderboardInfo) * m_MaxLeaderboards);// , __FILE__, __LINE__ );
    }
}
...
YYEXPORT void /*double*/ steam_create_leaderboard(RValue& Result, CInstance* selfinst, CInstance* otherinst, int argc, RValue* arg)//(const char* pLeaderboardName, double sortMethod, double displayType)/*Steam_UserStats_CreateLeaderboard*/
{
...
bool OnUploadScoreResult(LeaderboardScoreUploaded_t* pCallback, bool bIOFailure, CLeaderboardUploadHandler* pHandler)
{
    ...
    else if (!pCallback->m_bSuccess)
    {
        ...
        //BUT- fails when many uploads are attempted in a short space of time - perhaps being throttled by steam; and retries will also fail, until a certain amount of time has passed
        //-so repeatedly retrying is a really bad idea, as it will likely prevent further posts...
    ...
    /*if(bRetry)
    {
        ...
        SteamAPICall_t hSteamAPICall = SteamUserStats()->UploadLeaderboardScore( pCallback->m_hSteamLeaderboard, k_ELeaderboardUploadScoreMethodKeepBest, pCallback->m_nScore, NULL, 0 );
        pHandler->SetCallResult( hSteamAPICall );
        return false;    //re-using this handler
    }*/
    //and return async event instead
    int iSuccess = (bSuccess) ? 1 : 0;
    int iUpdated = (bUpdated) ? 1 : 0;
    int dsMapIndex = CreateDsMap(6,
        "event_type", (double)0.0, "leaderboard_upload",
        "lb_name", (double)0.0, pszName,
        "success", (double)iSuccess, NULL,
        "updated", (double)iUpdated, NULL,
        "post_id", (double)pHandler->m_postId, NULL,
        "score", (double)pCallback->m_nScore, NULL);
...
double Steam_UserStats_UploadScore(const char* pszLeaderboardName, double score, int _bufferId, bool _forceupdate)
{
    ...
    if (GetLeaderboardHandle(pszLeaderboardName, hLeaderboard))
    {
        ...
        if (_forceupdate)
            hSteamAPICall = SteamUserStats()->UploadLeaderboardScore(hLeaderboard, k_ELeaderboardUploadScoreMethodForceUpdate, (int32)score, pExtraData, dataCount);
        else
            hSteamAPICall = SteamUserStats()->UploadLeaderboardScore(hLeaderboard, k_ELeaderboardUploadScoreMethodKeepBest, (int32)score, pExtraData, dataCount);
...
YYEXPORT void /*double*/ steam_upload_score(RValue& Result, CInstance* selfinst, CInstance* otherinst, int argc, RValue* arg)//(const char* pszLeaderboardName, double score)
{
...
YYEXPORT void /*double*/ steam_upload_score_ext(RValue& Result, CInstance* selfinst, CInstance* otherinst, int argc, RValue* arg)//(const char* pszLeaderboardName, double score, bool _forceupdate)
{
...
YYEXPORT void /*double*/ steam_upload_score_buffer(RValue& Result, CInstance* selfinst, CInstance* otherinst, int argc, RValue* arg)//(const char* pszLeaderboardName, double score, int _bufferId)
{
    const char* pszLeaderboardName = YYGetString(arg, 0);
    double score = YYGetReal(arg, 1);
...
YYEXPORT void /*double*/ steam_upload_score_buffer_ext(RValue& Result, CInstance* selfinst, CInstance* otherinst, int argc, RValue* arg)//(const char* pszLeaderboardName, double score, int _bufferId, bool _forceupdate)
{
    const char* pszLeaderboardName = YYGetString(arg,0);
    double score = YYGetReal(arg,1);
...
YYEXPORT void /*double*/ steam_download_scores_around_user(RValue& Result, CInstance* selfinst, CInstance* otherinst, int argc, RValue* arg)//( const char* pszLeaderboardName, double rangeStart, double rangeEnd )/*Steam_UserStats_DownloadScoresAroundUser*/
{
    const char* pszLeaderboardName = YYGetString(arg,0);
    ...
    {
        CLeaderboardDownloadHandler* pResultHandler = new CLeaderboardDownloadHandler(async_id);
        SteamAPICall_t hSteamAPICall = SteamUserStats()->DownloadLeaderboardEntries(hLeaderboard, k_ELeaderboardDataRequestGlobalAroundUser, (int)rangeStart, (int)rangeEnd );
        pResultHandler->SetCallResult( hSteamAPICall );
    }
    else
    {
...
YYEXPORT void /*double*/ steam_download_friends_scores(RValue& Result, CInstance* selfinst, CInstance* otherinst, int argc, RValue* arg)// const char* pszLeaderboardName )/*Steam_UserStats_DownloadFriendsScores*/
{
    const char* pszLeaderboardName = YYGetString(arg, 0);
...
YYEXPORT void /*double*/ steam_set_achievement(RValue& Result, CInstance* selfinst, CInstance* otherinst, int argc, RValue* arg)//(const char* pAchievementID)/*Steam_UserStats_SetAchievement*/
{
...
YYEXPORT void /*double*/ steam_get_achievement(RValue& Result, CInstance* selfinst, CInstance* otherinst, int argc, RValue* arg)//(const char* pAchievementID)/*Steam_UserStats_GetAchievement*/
{
    const char* pAchievementID = YYGetString(arg, 0);
...
YYEXPORT void /*double*/ steam_clear_achievement(RValue& Result, CInstance* selfinst, CInstance* otherinst, int argc, RValue* arg)//(const char* pAchievementID)/*Steam_UserStats_ClearAchievement*/
{
    const char* pAchievementID = YYGetString(arg, 0);
...
YYEXPORT void /*double*/ steam_set_stat_int(RValue& Result, CInstance* selfinst, CInstance* otherinst, int argc, RValue* arg)//(const char* pStatName, double val)/*Steam_UserStats_SetStatInt*/
{
...
YYEXPORT void /*double*/ steam_set_stat_float(RValue& Result, CInstance* selfinst, CInstance* otherinst, int argc, RValue* arg)//(const char* pStatName, double val)/*Steam_UserStats_SetStatFloat*/
{
    const char* pStatName = YYGetString(arg,0);
    double val = YYGetReal(arg,1);
...
YYEXPORT void /*double*/ steam_set_stat_avg_rate(RValue& Result, CInstance* selfinst, CInstance* otherinst, int argc, RValue* arg)//(const char* pStatName, double dCountThisSession, double dSessionLength)/*Steam_UserStats_SetStatAvgRate*/
{
    const char* pStatName = YYGetString(arg, 0);
...
YYEXPORT void /*double*/ steam_get_stat_int(RValue& Result, CInstance* selfinst, CInstance* otherinst, int argc, RValue* arg)//(const char* pStatName)/*Steam_UserStats_GetStatInt*/
{
    const char* pStatName = YYGetString(arg,0);
...
YYEXPORT void /*double*/ steam_get_stat_float(RValue& Result, CInstance* selfinst, CInstance* otherinst, int argc, RValue* arg)//(const char* pStatName)/*Steam_UserStats_GetStatFloat*/
{
    const char* pStatName = YYGetString(arg, 0);
    if (!steam_is_initialised)
    {
...
YYEXPORT void /*double*/ steam_get_stat_avg_rate(RValue& Result, CInstance* selfinst, CInstance* otherinst, int argc, RValue* arg)//(const char* pStatName)/*Steam_UserStats_GetStatAvgRate*/
{
    const char* pStatName = YYGetString(arg, 0);
